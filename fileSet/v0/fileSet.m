classdef fileSet < handle
    ...
    methods
        ...
        function buildFileSetFromConfig(obj,inputFileName)
            ...
            % load folders info from Config file
            obj.loadFolderInfoFromConfig;
        ...
        function buildFileSetFromFList(obj,fList,conditions,outFolder)
            
            obj.fList = fList;
            obj.conditions = reshape(conditions,1,numel(conditions));
            obj.fileTypes = fList.Properties.VariableNames(...
                ~ismember(fList.Properties.VariableNames,conditions));
        ...
        function status = loadOptionsFromConfig(obj)
            ...
            if isempty(obj.iniFileName)
                disp('Config file name missing, cannot load options.');
                status = 0;
                return
            end
        ...
        function loadConditionsFromConfig(obj)
            if isempty(obj.iniFileName)
                disp('Error: Config file name missing, cannot load conditions.');
                return
            end
            
            % parse list of conditions from ini config file
            obj.readListEntryFromConfig('conditions','Files','conditions');
            
            if ~obj.options.batchMode
            ...
            if isempty(obj.conditions)
%                 disp(['Warning: No condition List were found in the config file: ',...
%                     obj.iniFileName,';']);
%                 disp('    All files in input folder will be loaded.');
%                 disp(['    -> if incorrect, make sure the field ',...
%                     '''conditions = '' is populated in config file']);
            else
        ...
        function loadFileTypesFromConfig(obj)
            if isempty(obj.iniFileName)
                disp('Error: Config file name missing, cannot load fileTypes.');
                return
            end
            
            % parse list of file Types from ini config file
            obj.readListEntryFromConfig('fileTypes','Files','fileTypes');
            if isempty(obj.fileTypes)
                disp(['Error: Could not parse the list of file Types in config file: ',...
                    obj.iniFileName,';']);
                disp(['    -> make sure the field ''fileTypes = '' is populated ',...
                    'in config file.']);
                return
            end
        ...
        function loadFolderInfoFromConfig(obj)
            if isempty(obj.iniFileName)
                disp('Error: Config file name missing, cannot load fileTypes.');
                return
            end
        ...
        function status = loadPatternSetFromConfig(obj)
            ...
            if isempty(obj.iniFileName)
                disp('Error: Config file name missing, cannot create dataset.');
                status = 0;
                return
            end
        ...
        function status = assembleFileListFromPattern(obj)
            ...
            if ~obj.checkBatchModeFromConfig
                ...
                % if batch mode, collect all files
                obj.fList = [];
                obj.conditions = [];
                for i=1:numel(obj.fileTypes)
                    [ftmp,ctmp] = ...
        ...
        %% returns number of conditions of type conditionName
        ...
        %% returns list of conditions of type conditionName
        ...
        function status = loadSinglePatternFromConfig(obj,fileType)
            if isempty(obj.iniFileName)
                disp('Error: Config file name missing, cannot load file Pattern.');
                status = 0;
                return
            end
            ...
            p = GetValues(inConf, ...
                'Files', [fileType,'Pattern'], 'fail');
            if strcmp(p,'fail') 
                p = GetValues(inConf, ...
                'Files', [fileType,'pattern'], 'fail');
                if strcmp(p,'fail') 
                    disp(['Error: File type ',fileType,...
                        ' has no matching file pattern in config file: ',...
                        obj.iniFileName]);
                    disp(['    -> make sure the field ''',fileType,...
                        'Pattern = '' is populated in the config file.']);
                    status = 0;
                    return
                end
            end
        ...
        % set of conditions
        function fname = collectFileName(obj,...
        ...
        % example use: 
        % fname = collectFileName(obj.fList,{'FOV','Round'},{2,3},'TransMat','all')
        ...
        idx = true(size(obj.fList,1),1);
        for i=1:numel(conditionNames)
            t = table(conditionValues{i},'VariableNames',conditionNames(i));
            idx = idx & ismember(obj.fList(:,idxCon(1,i)), t );
        end

        if sum(idx) == 0
            disp(['Could not find a ',desiredVarName,...
        ...
        % will be returned
        if sum(idx) > 1 && strcmp( processMultiple, 'first')
            idx = find(idx,1);   
        end

        fname = obj.fList.(desiredFileType)(idx);
         if numel(fname)>1
             fname = unique(fname);
         end
        ...
        function fillFileName(obj,...
            ...
            obj.fList.(desiredFileType)(idx) = {fileNameEntry};
        ...
        function [isCommon,isUnique,fCommon,fUnique] = extractCommonConditions(~,f,conditionList,commonConds)
            isCommon = ismember(f.Properties.VariableNames,commonConds);
            fCommon = f(:, f.Properties.VariableNames(isCommon));
            isUnique =  ismember(f.Properties.VariableNames,setdiff(conditionList,commonConds));
    ...
    methods (Access = 'private')
        ...
        %% merging tables of files flist and ftmp, 
        % with respective list of conditions conditions 
